...
</h2></div><div class="contact-intro"><p class="lead">${esc(site.contact.lead)}</p>${mail}${socials}</div></div><form class="contact-form reveal" id="contactForm" novalidate><input type="hidden" name="access_key" value="${attr(site.contact.web3formsAccessKey)}"><input type="hidden" name="subject" value="Personal gym Recipeへのお問い合わせ"><input type="checkbox" name="botcheck" class="botcheck" tabindex="-1" autocomplete="off"><div class="contact-form-grid two-col"><div class="field"><label for="name">お名前 *</label><input id="name" name="name" required autocomplete="name" placeholder="山田 太郎"></div><div class="field"><label for="email">メールアドレス *</label><input id="email" type="email" name="email" required autocomplete="email" placeholder="example@email.com"></div></div><div class="field"><label for="phone">電話番号（任意）</label><input id="phone" type="tel" name="phone" autocomplete="tel" inputmode="tel" placeholder="090-1234-5678"></div><div class="field message-field"><label for="message">お問い合わせ内容 *</label><textarea id="message" name="message" required placeholder="体験希望、ご相談内容などをご記入ください"></textarea></div><label class="privacy-consent"><input id="privacyConsent" type="checkbox" required> <span>入力された情報は、お問い合わせへの回答のために使用することに同意します。</span></label><div class="form-bottom"><button class="submit" id="confirmContact" type="button">入力内容を確認する</button><div class="form-note">送信前に入力内容を確認できます。</div></div><div class="contact-review" id="contactReview" hidden><h3>入力内容をご確認ください</h3><dl id="contactSummary"></dl><div class="review-actions"><button type="button" id="editContact">入力内容を修正する</button><button class="submit" id="sendContact" type="submit">この内容で送信する</button></div></div><p class="form-status" id="formStatus" role="status" aria-live="polite"></p></form></div></section>

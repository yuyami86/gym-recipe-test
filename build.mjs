...
</h2><p>${esc(site.trainer.body)}</p><div class="stats"><div class="stat"><strong class="serif">${esc(site.trainer.years)}</strong><span>YEARS / トレーナー歴</span></div><div class="stat"><strong class="serif">${esc(site.trainer.core)}</strong><span>CORE STYLE / 指導の中心</span></div><div class="stat"><strong class="serif">${esc(site.trainer.specialty)}</strong><span>WEIGHT / 得意分野</span></div></div></div></div></section>
<section class="section price" id="price"><div class="container"><div class="section-head reveal"><div class="eyebrow">04 / Price</div><h2 class="serif mobile-one-line">続け方も、あなたに合わせる。</h2><p class="lead">目的やトレーニング経験に合わせて、無理のないペースから始められます。</p></div><div class="price-grid">${plans}</div><div class="counseling-band reveal"><span class="label">FIRST COUNSELING</span><strong>${esc(site.counseling.title)}</strong><p>${esc(site.counseling.body)}</p></div></div></section>
...
<section class="section contact" id="contact"><div class="container"><div class="contact-head reveal"><div class="contact-title-block"><div class="eyebrow">06 / Contact</div><h2 class="serif">
